...
export const handleRegister = async ({ interaction, forumChannel, client }) => {
    try {
        await interaction.deferReply();
        const guildName = interaction.options.getString('문파명', true).toLowerCase();
        const jobTarget = interaction.options.getString('직업명', true);
        const nickname = interaction.options.getString('캐릭터닉네임', true).trim();
        // 1. 문파 스레드 검색
        ...
        // 5. 성공 메시지 반환
        await interaction.editReply(`[${targetThread.name}](${createThreadUrl(interaction.guildId, targetThread.id)})의 **${cleanJobName}** 명단에 "${nickname}" 님을 추가했습니다!`);
    }
    catch (error) {
        ...
        if (interaction.deferred || interaction.replied) {
            ...
            await interaction.reply({ content: '등록 수정 중 오류가 발생했습니다.', ephemeral: true }).catch(() => { });
        }
    }
};
