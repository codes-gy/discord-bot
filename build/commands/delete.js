...
export const handleDelete = async ({ interaction, forumChannel, client }) => {
    try {
        // 응답 지연 처리 (3초 제한 연장)
        ...
        // 5. 성공 메시지 반환
        await interaction.editReply(`[${targetThread.name}](${createThreadUrl(interaction.guildId, targetThread.id)}) 명단에서 "${nickname}" 님을 성공적으로 삭제했습니다.`);
    }
    catch (error) {
