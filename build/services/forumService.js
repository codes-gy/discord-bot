...
import { MEMBER_LIST_MARKER, } from '../types/forumType';
export async function getForumChannel(client, forumChannelId) {
 ...
 */
export async function getAllThreads(forumChannel) {
    const emptyThreads = { threads: new Collection() };
...
export async function findThreadByName(forumChannel, keyword) {
...
export async function findMemberListMessage({ thread }) {
...
export async function saveMemberListMessage({ client, thread, targetMessage, updatedContent }) {
...
export function createThreadUrl(guildId, threadId) {
    return `https://discord.com/channels/${guildId ?? '@me'}/${threadId}`; // guildId가 없을 때(DM 등)를 위한 방어 코드 추가
}
export function addNicknameToContent({ content, jobTarget, nickname }) {
...
export function removeNicknameFromContent({ content, nickname }) {
    if (!hasExactNickname(content, nickname)) {
        return {
            success: false,
...
function splitNicknames(line) {
    return line.trim().split(/\s+/).filter(Boolean); // 미리 trim 수행하여 사이드 이펙트 방지
}
function hasExactNickname(content, nickname) {
